...
// export default function PageTester() {
// 	return (
// 		<>
// 			<Navbar></Navbar>
...
export default function PageTester() {
	...
	const fetchPostData = async () => {
		try {
			const res = await axios.post("http://localhost:3000/get/post", {
				params: { page, limit: 3 },
			});
			console.log(res);
			...
			console.log("🚀 ~ fetchPostData ~ page:", page);

			if (res.data.data.length === 0 || res.data.data.length < 3) {
				setHasMore(false);
			}
		} catch (err) {
			console.error(err);
		...
		<>
			<div className="xl:flex mt-[61px] md:mt-[73px] xl:ml-[288px] xl:justify-between xl:mr-[28%]">
				<InfiniteScroll
					...
					endMessage={
						<p className="dynamic-text align-middle">
				...
				>
					{items.map((item, index) => (
						<div
							key={index}
							className="h-[50px] border-2 border-green dynamic-text"
						>
